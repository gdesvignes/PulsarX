 ...
 * @create date 2023-02-04 14:14:27
 * @modify date 2023-02-04 14:14:27
 * @desc [description]
 */
...
int main(int argc, const char *argv[])
{
	...
	desc.add_options()
			...
			("scale", value<int>()->default_value(1), "F0,F1,dm search range scale in phase")
	                ("rmstep", value<double>()->default_value(5), "RM step (rad/m/m)")
	                ("rmlim", value<double>()->default_value(10000), "Minimum/maximum RM value (rad/m/m)")
			...
			("nsubband,n", value<int>()->default_value(32), "Number of subband")
	                ("pscr", value<int>()->default_value(1), "Frequency scrunch for the plot")
	...
	DataBuffer<float> databufIQUV(ndump, nchans, nifs);
	DataBuffer<float> databufI(ndump, nchans);
	DataBuffer<float> databufQ(ndump, nchans);
	DataBuffer<float> databufU(ndump, nchans);
	DataBuffer<float> databufV(ndump, nchans);
	databufI.closable = true; databufQ.closable = true; databufU.closable = true; databufV.closable = true;
	databufI.tsamp = tsamp; databufQ.tsamp = tsamp; databufU.tsamp = tsamp ;databufV.tsamp = tsamp;
	memcpy(&databufIQUV.frequencies[0], reader->frequencies.data(), sizeof(double)*nchans);
	memcpy(&databufI.frequencies[0], reader->frequencies.data(), sizeof(double)*nchans);
	memcpy(&databufQ.frequencies[0], reader->frequencies.data(), sizeof(double)*nchans);
	memcpy(&databufU.frequencies[0], reader->frequencies.data(), sizeof(double)*nchans);
	memcpy(&databufV.frequencies[0], reader->frequencies.data(), sizeof(double)*nchans);
	...
	Patch patchI;
	patchI.filltype = vm["fillPatch"].as<string>();
	patchI.width = vm["widthPatch"].as<float>();
	patchI.threshold = vm["threPatch"].as<float>();
	Patch patchQ = patchI; Patch patchU = patchI; Patch patchV = patchI;
	patchI.prepare(databufI); patchQ.prepare(databufQ); patchU.prepare(databufU); patchV.prepare(databufV);
	patchI.close();patchQ.close();patchU.close();patchV.close();

	PreprocessLite prepI;
	prepI.td = vm["td"].as<int>();
	prepI.fd = vm["fd"].as<int>();
	prepI.thresig = vm["zapthre"].as<float>();
	prepI.filltype = vm["fill"].as<string>();
	PreprocessLite prepQ = prepI; PreprocessLite prepU = prepI; PreprocessLite prepV = prepI;
	prepI.prepare(databufI); prepQ.prepare(databufQ); prepU.prepare(databufU); prepV.prepare(databufV);

	Equalize equalizeI, equalizeQ, equalizeU, equalizeV;
	equalizeI.prepare(prepI); equalizeQ.prepare(prepQ); equalizeU.prepare(prepU); equalizeV.prepare(prepV);
	equalizeI.close(); equalizeQ.close(); equalizeU.close(); equalizeV.close();
	equalizeI.closable = true; equalizeQ.closable = true; equalizeU.closable = true; equalizeV.closable = true;

	BaseLine baselineI;
	baselineI.width = vm["baseline"].as<vector<float>>().back();
	BaseLine baselineQ=baselineI; BaseLine baselineU=baselineI; BaseLine baselineV=baselineI;
	baselineI.prepare(equalizeI); baselineQ.prepare(equalizeQ); baselineU.prepare(equalizeU); baselineV.prepare(equalizeV);
	baselineI.close(); baselineQ.close(); baselineU.close(); baselineV.close();
	baselineI.closable = true; baselineQ.closable = true; baselineU.closable = true; baselineV.closable = true;

	RFI rfiI;
	rfiI.filltype = vm["fill"].as<string>();
	RFI rfiQ=rfiI; RFI rfiU=rfiI; RFI rfiV=rfiI;
	rfiI.prepare(baselineI); rfiQ.prepare(baselineQ); rfiU.prepare(baselineU); rfiV.prepare(baselineV);
	rfiI.close(); rfiQ.close(); rfiU.close(); rfiV.close();
	rfiI.closable = true; rfiQ.closable = true; rfiU.closable = true; rfiV.closable = true;
	...
	std::vector<std::pair<std::string, std::string>> meta = {
		{"nsamples", std::to_string(rfiI.nsamples)},
		{"nchans", std::to_string(rfiI.nchans)},
		{"tsamp", std::to_string(rfiI.tsamp)},
		{"rfi_flags", rfi_flags},
	...
	else if (vm.count("parfile"))
	{
		...
		for (long int k=0; k<folder.size(); k++)
		{
			...
			double fmin = databufI.frequencies[0];
			double fmax = databufI.frequencies[0];
			for (long int j=0; j<nchans; j++)
			{
				fmin = fmin<databufI.frequencies[j] ? fmin:databufI.frequencies[j];
				fmax = fmax>databufI.frequencies[j] ? fmax:databufI.frequencies[j];
			}
			double chanwidth = abs(databufI.frequencies[1]-databufI.frequencies[0]);
			fmin -= 0.5*chanwidth;
			fmax += 0.5*chanwidth;
	...
	int fscrunch;
	dedisp.nsubband = nchans;
	int nsub = vm["nsubband"].as<int>();
	fscrunch = (int) (nchans / nsub);
	double maxdm = 0;
	...
	Pulsar::DedispersionLiteU dedispQ = dedisp;
	Pulsar::DedispersionLiteU dedispU = dedisp;
	Pulsar::DedispersionLiteU dedispV = dedisp;
	dedisp.prepare(rfiI);
	dedispQ.prepare(rfiQ);
	dedispU.prepare(rfiU);
	dedispV.prepare(rfiV);

	DataBuffer<float> subdata, subdataQ, subdataU, subdataV, subdataIQUV;
	dedisp.get_subdata(subdata, 0);
	dedispQ.get_subdata(subdataQ, 0);
	dedispU.get_subdata(subdataU, 0);
	dedispV.get_subdata(subdataV, 0);
	...
	for (long int k=0; k<ncand; k++)
	{
		...
		folder[k].resize(4, subdata.nchans, folder[k].nbin);
	...
	while (!reader->is_end)
	{
		if (reader->read_data(databufIQUV, ndump) != ndump) break;

		databufI.get_pol(databufIQUV, 0);
		databufQ.get_pol(databufIQUV, 1);
		databufU.get_pol(databufIQUV, 2);
		databufV.get_pol(databufIQUV, 3);
		databufI.counter += ndump;

		DataBuffer<float> *data = patchI.filter(databufI);
		DataBuffer<float> *dataQ = patchQ.filter(databufQ);
		DataBuffer<float> *dataU = patchU.filter(databufU);
		DataBuffer<float> *dataV = patchV.filter(databufV);

		data = prepI.run(*data);
		dataQ = prepQ.run(*dataQ);
		dataU = prepU.run(*dataU);
		dataV = prepV.run(*dataV);

		data = equalizeI.filter(*data);
		dataQ = equalizeQ.filter(*dataQ);
                dataU = equalizeU.filter(*dataU);
                dataV = equalizeV.filter(*dataV);
		
		data = baselineI.filter(*data);
		dataQ = baselineQ.filter(*dataQ);
		dataU = baselineU.filter(*dataU);
		dataV = baselineV.filter(*dataV);

		data = rfiI.zap(*data, zaplist);
		dataQ = rfiQ.zap(*dataQ, zaplist);
		dataU = rfiU.zap(*dataU, zaplist);
		dataV = rfiV.zap(*dataV, zaplist);
		
		if (rfiI.isbusy) {rfiI.closable = false; rfiQ.closable = false; rfiU.closable = false; rfiV.closable = false;}

		for (auto irfi = rfilist.begin(); irfi!=rfilist.end(); ++irfi)
		{
			if ((*irfi)[0] == "mask")
			{
				data = rfiI.mask(*data, threMask, stoi((*irfi)[1]), stoi((*irfi)[2]));
				if (rfiI.isbusy) rfiI.closable = false;
			}
			else if ((*irfi)[0] == "kadaneF")
			{
				data = rfiI.kadaneF(*data, threKadaneF*threKadaneF, widthlimit, stoi((*irfi)[1]), stoi((*irfi)[2]));
				dataQ = rfiQ.kadaneF(*dataQ, threKadaneF*threKadaneF, widthlimit, stoi((*irfi)[1]), stoi((*irfi)[2]));
				dataU = rfiU.kadaneF(*dataU, threKadaneF*threKadaneF, widthlimit, stoi((*irfi)[1]), stoi((*irfi)[2]));
				dataV = rfiV.kadaneF(*dataV, threKadaneF*threKadaneF, widthlimit, stoi((*irfi)[1]), stoi((*irfi)[2]));
				if (rfiI.isbusy) {rfiI.closable = false; rfiQ.closable = false; rfiU.closable = false; rfiV.closable = false;}
			}
			else if ((*irfi)[0] == "kadaneT")
			{
				data = rfiI.kadaneT(*data, threKadaneT*threKadaneT, bandlimitKT, stoi((*irfi)[1]), stoi((*irfi)[2]));
				if (rfiI.isbusy) rfiI.closable = false;
			}
			else if ((*irfi)[0] == "zdot")
			{
				data = rfiI.zdot(*data); dataQ = rfiQ.zdot(*dataQ); dataU = rfiU.zdot(*dataU); dataV = rfiV.zdot(*dataV);
				if (rfiI.isbusy) {rfiI.closable = false; rfiQ.closable = false; rfiU.closable = false; rfiV.closable = false;}
			}
			else if ((*irfi)[0] == "zero")
			{
		...
		data->closable = true; dataQ->closable = true; dataU->closable = true; dataV->closable = true;
		dedisp.prerun(*data); dedispQ.prerun(*dataQ); dedispU.prerun(*dataU); dedispV.prerun(*dataV);
		...
		for (long int k=0; k<ncand; k++)
		{
			if (k%GROUPSIZE == 0)
			{
				...
				dedispQ.updatedm(dmsegs[k/GROUPSIZE]);
				dedispU.updatedm(dmsegs[k/GROUPSIZE]);
				dedispV.updatedm(dmsegs[k/GROUPSIZE]);
				dedisp.run(); dedispQ.run(); dedispU.run(); dedispV.run();
			}
			dedisp.get_subdata(subdata, k%GROUPSIZE);
			dedispQ.get_subdata(subdataQ, k%GROUPSIZE);
			dedispU.get_subdata(subdataU, k%GROUPSIZE);
			dedispV.get_subdata(subdataV, k%GROUPSIZE);

			if (dedisp.counter >= dedisp.offset+dedisp.ndump)
			{
			  subdataIQUV.four2one(subdata, subdataQ, subdataU, subdataV);
			  folder[k].runDspsr(subdataIQUV);
			}
		}
	 ...
	 * 
	 */
	BOOST_LOG_TRIVIAL(info)<<"Flushing the end of data"<<std::endl;
	int ngroup = dmsegs.size();

	int nleft = dedisp.offset/dedisp.ndump;
	for (long int l=0; l<nleft; l++)
	{
		rfiI.open(); rfiQ.open(); rfiU.open(); rfiV.open();
		dedisp.prerun(rfiI); dedispQ.prerun(rfiQ); dedispU.prerun(rfiU); dedispV.prerun(rfiV);
		for (long int k=0; k<ncand; k++)
		{
			if (k%GROUPSIZE == 0)
			{
	...
	double fmin = 1e6;
	double fmax = 0.;
	//BOOST_LOG_TRIVIAL(info)<<"Databuf I nchans: "<< databufI.nchans << " "<< databufI.frequencies[1] << " " << databufI.frequencies[2] << std::endl;
	for (long int j=0; j<databufI.nchans; j++)
	{
		fmax = databufI.frequencies[j]>fmax? databufI.frequencies[j]:fmax;
		fmin = databufI.frequencies[j]<fmin? databufI.frequencies[j]:fmin;
	}

	double tint = ntotal*tsamp;

	BOOST_LOG_TRIVIAL(info)<<"Preparing grid search, fmin="<<fmin << " fmax="<< fmax << " " << folder[0].f0 << std::endl;
...
#pragma omp parallel for num_threads(num_threads)
	...
	for (long int k=0; k<ncand; k++)
	{
	  cout << "Cand # " << k << " "<< endl;
		...
		gridsearch[k].rmstep = vm["rmstep"].as<double>();
		gridsearch[k].rmlim = vm["rmlim"].as<double>();
	...
	obsinfo["Fcentre"] = to_string(0.5*(databufI.frequencies.front()+databufI.frequencies.back()));
	obsinfo["Bandwidth"] = to_string((databufI.frequencies.back()-databufI.frequencies.front())/(databufI.frequencies.size()-1)*databufI.frequencies.size());
	obsinfo["Nchan"] = to_string(databufI.frequencies.size());
	...
	outfile<<"#id       dm_old      dm_new      dm_err		dist_ymw16     f0_old     f0_new        f0_err      f1_old     f1_new       f1_err      acc_old        acc_new      acc_err      S/N        S/N_new        RM"<<endl;
	...
	for (long int k=0; k<ncand; k++)
	{
		...
		outfile<<fixed<<setprecision(6)<<gridsearch[k].bestRM<<endl;
